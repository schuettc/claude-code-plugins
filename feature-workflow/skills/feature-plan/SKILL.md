...
- `/feature-submit` opens a draft PR for external review
...
1. **`/feature-implement [feature-id]`** — write code following the plan
2. **`/feature-submit [feature-id]`** — create a feature branch, open a draft PR, trigger external reviewers (Gemini/Codex)
3. **`/feature-submit [feature-id] --respond`** — address review feedback (repeat as needed)
4. **`/feature-ship [feature-id]`** — write shipped.md, merge the PR into `dev`, clean up the branch

**Do NOT skip `/feature-submit`** — it's the step that opens the draft PR for external review. `/feature-ship` is the final merge step after reviews are satisfactory.
