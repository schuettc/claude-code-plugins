...
- **`project-init`** *(user-invocable)* — interactively apply the right subset of these patterns to the current repo. Detects what's already in place and applies only the gaps, so it works on new repos and existing ones alike. Also offers a **multi-repo workspace on-ramp**: when the repo is (or should be) a coordination hub for several interconnected repos, it scaffolds the workspace via [`feature-workflow`](../feature-workflow)'s `feature-init --workspace` and points each member back through `/project-init`.
