...
Copy the example config template into the project's `.claude/` directory:

```bash
cp "${CLAUDE_PLUGIN_ROOT}/skills/setup-ghost/ghost.local.example.md" .claude/ghost.local.md
```
